...
class SelectedSLIdPage extends StatefulWidget {
  const SelectedSLIdPage({super.key, required this.title});
  final String title;

  @override
  State<SelectedSLIdPage> createState() => _SelectedIdPageState();
}


class _SelectedIdPageState extends State<SelectedSLIdPage> {
  @override
...
class _SelectedListBodyState extends State<_SelectedListBody> {
  ...
  Future<void> _removeAt(int index) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Slope ID removed'), duration: Duration(milliseconds: 700)),
      );
    } catch (e) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text('No selected Slope IDs', style: TextStyle(fontSize: 18, color: Colors.grey)),
              SizedBox(height: 8),
              Text(
