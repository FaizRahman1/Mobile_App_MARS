...
class _RecordedInspectionState extends State<RecordedInspection> {
  ...
  Future<void> _load() async {
    ...
    List<DRPostModel> list = <DRPostModel>[];
    try {
      if (raw != null && raw.trim().isNotEmpty) {
        list = DRPostModel.decode(raw);
      }
    } catch (_) {
  ...
  Future<void> _delete(DRPostModel item) async {
    ...
    final list = DRPostModel.decode(raw);
    list.removeWhere((x) => x.id == item.id);
    await prefs.setString('info', DRPostModel.encode(list));

    if (!mounted) return;
    await _load();
  }

  void _review(DRPostModel item) {
    Navigator.push(
      context,
...
Future<void> _edit(DRPostModel item) async {
  ...
  void _goSearch() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (_) => DrainageidlistPage(id: "")),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Inspected Drainage'),
        centerTitle: true,
        actions: [
          IconButton(
            tooltip: 'Refresh',
