...
class InspectionDetailPage extends StatelessWidget {
  ...
  Widget _generalInspectionImages(BuildContext context) {
    ...
    if (images.isEmpty) {
      return _section('General Inspection Images', const [
        SizedBox(height: 6),
        Text('No general inspection images were saved from the web system.'),
      ]);
    }

    return _section('General Inspection Images', [
      const Align(
      ...
      Wrap(
        ...
        children: List.generate(images.length, (index) {
          ...
          final caption = index < (inspection.imageCaptions?.length ?? 0)
              ? inspection.imageCaptions![index].trim()
              : '';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Review Slope: ${_value(inspection.id)}')),
      body: ListView(
        ...
        children: [
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  _keyValue('Slope ID', _value(inspection.id)),
                  _keyValue(
                    'Date of Inspection',
                    _formatDate(inspection.dateofinsp),
                  ),
                  _keyValue('Inspected By', _value(inspection.inspectedby)),
                  _keyValue('Maintained By', _value(inspection.maintainedby)),
                ],
              ),
            ),
          ),
          const SizedBox(height: 10),
          _section('General Slope Data', [
            _keyValue('Disk/Film No', _value(inspection.diskfilmno)),
            _keyValue('Photo No', _value(inspection.photono)),
            _keyValue(
              'Interface Location',
              _value(inspection.interfacelocation),
            ),
            _keyValue('Accessibility', _value(inspection.accessibility)),
            _keyValue('Accessibility Why', _value(inspection.accessibilitywhy)),
          ], initiallyExpanded: true),
          _section('Routine Works Checked', [
            _keyValue(
            ...
            const Divider(),
            _keyValue('Gully Repair Form', _value(inspection.gullyrepairform)),
            _keyValue(
            ...
            _keyValue(
              ...
              _value(inspection.precastconcretereplacementform),
            ),
            _keyValue(
              'Precast Concrete Replacement Form Description',
              _value(inspection.precastconcretereplacementformdesc),
            ),
            const Divider(),
            _keyValue(
            ...
            _keyValue(
              ...
              _value(inspection.earthdrainresectioningformdesc),
            ),
            const Divider(),
            _keyValue(
              'Other Routine Work',
            ...
            _keyValue('Status RM', _value(inspection.statusrm)),
          ]),
