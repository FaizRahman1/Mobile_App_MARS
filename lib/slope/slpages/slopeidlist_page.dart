...
import 'package:bridgeinsp_new/slope/slpages/selectedid_page.dart';

import '../bloc/slopeidlist_bloc.dart';

class SlopeidlistPage extends StatefulWidget {
  final String id;

  const SlopeidlistPage({super.key, required this.id});

  @override
  State<SlopeidlistPage> createState() => _SlopeidlistPageState();
}

class _SlopeidlistPageState extends State<SlopeidlistPage> {
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  final SlopeidlistBloc _bloc = SlopeidlistBloc();

  @override
  void initState() {
    super.initState();
    _bloc.add(GetSlopeidList(id: widget.id));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Search Slope ID'),
        centerTitle: true,
        actions: [
          IconButton(
            tooltip: 'Refresh',
            icon: const Icon(Icons.refresh),
            onPressed: () => _bloc.add(GetSlopeidList(id: widget.id)),
          ),
      ...
      body: BlocProvider.value(
        value: _bloc,
        child: BlocListener<SlopeidlistBloc, SlopeidlistState>(
          listener: (context, state) {
            if (state is SlopeidlistError) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(state.message ?? 'Error')),
              );
            }
          },
          child: BlocBuilder<SlopeidlistBloc, SlopeidlistState>(
            builder: (context, state) {
              if (state is SlopeidlistInitial || state is SlopeidlistLoading) {
                return const Center(child: CircularProgressIndicator());
              }

              if (state is SlopeidlistLoaded) {
                final List<Rows> allRows =
                   (state.slopeidlistModel.rows ?? []).cast<Rows>();
                return _SlopeListContent(
...
class _SlopeListContent extends StatefulWidget {
  final List<Rows> originalData;
  final String pageId;
  final GlobalKey<ScaffoldState> scaffoldKey;

  const _SlopeListContent({
  ...
  @override
  State<_SlopeListContent> createState() => _SlopeListContentState();
}

class _SlopeListContentState extends State<_SlopeListContent> {
  ...
  @override
  void didUpdateWidget(covariant _SlopeListContent oldWidget) {
...
class _SlopeIdCard extends StatelessWidget {
  final Rows row;
  final GlobalKey<ScaffoldState> scaffoldKey;
  ...
  Future<void> _handleTap(BuildContext context) async {
    final confirmed = await showCupertinoDialog<bool>(
      ...
      builder: (_) => CupertinoAlertDialog(
        title: const Text('Select this slope ID?'),
        content: Text(row.id ?? '-'),
        actions: [
    ...
    final ok = await SelectedslopeStore.upsert(row);
    if (!ok) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Failed to save selected slope.')),
      );
      return;
    }

    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Selected slope saved'), duration: Duration(milliseconds: 700)),
    );
    ...
    await showCupertinoDialog<void>(
      ...
      builder: (_) => CupertinoAlertDialog(
        ...
        content: const Text('Slope ID successfully selected.'),
        actions: [
          CupertinoDialogAction(
          ...
          CupertinoDialogAction(
            onPressed: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const SelectedSLIdPage(title: 'List of Selected Inspection'),
                ),
              );
            },
...
class SelectedslopeStore {
  static const String kIdsKey = 'selected_slope_ids';
  static const String kRowsJsonKey = 'selected_slope_rows_json';
