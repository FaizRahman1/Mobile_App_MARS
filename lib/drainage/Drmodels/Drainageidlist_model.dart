...
class DrainageidlistModel {
  ...
  DrainageidlistModel.withError(String errorMessage) {
    error = errorMessage;
  }

  // --- REMOVED SORTING FROM HERE ---
  DrainageidlistModel.fromJson(Map<String, dynamic> json) {
  ...
  factory DrainageidlistModel.fromJsonn(Map<dynamic, dynamic> json) {
    List<Rows> rows = [];
  ...
  static Map<String, dynamic> toMap(DrainageidlistModel model) => {
  ...
  static String encode(List<DrainageidlistModel>? list) => json.encode(
        list!
            .map<Map<String, dynamic>>(
                (model) => DrainageidlistModel.toMap(model))
            .toList(),
      );

  static List<DrainageidlistModel> decode(String list) =>
      (json.decode(list) as List<dynamic>)
          .map<DrainageidlistModel>(
              (item) => DrainageidlistModel.fromJson(item)) // Uses the modified fromJson
          .toList();
}
