...
class ApiProviderBridgeinventory {
  final String? url;
  final Dio _dio = Dio();

  ApiProviderBridgeinventory({required this.url});

  Future<BridgeinventoryModel> fetchBridgeinv() async {
    try {
      String inv = url!.replaceAll(r'/', '%2F');
      String newurl =
          'https://teman-api.plus.com.my/c&s/bridgegi/all?id=$inv';
      ...
      return BridgeinventoryModel.fromJson(
          (response.data) as Map<String, dynamic>);
    } catch (error, stacktrace) {
      if (kDebugMode) {
        print("Exception occured: $error stackTrace: $stacktrace");
      }
      return BridgeinventoryModel.withError(
