...
import 'package:bridgeinsp_new/slope/slpages/inspection_detail_page.dart';
...
class _RecordedInspectionState extends State<RecordedInspection> {
  ...
  bool _loading = true;
  List<SLPostModel> _all = <SLPostModel>[];
  ...
  Future<void> _load() async {
    ...
    List<SLPostModel> list = <SLPostModel>[];
    try {
      if (raw != null && raw.trim().isNotEmpty) {
        list = SLPostModel.decode(raw);
      }
    } catch (_) {
  ...
  List<SLPostModel> get _filtered {
    final q = _searchCtrl.text.trim().toLowerCase();
    if (q.isEmpty) return _all;
    return _all.where((x) => (x.id ?? '').toLowerCase().contains(q)).toList();
  }

  Future<void> _delete(SLPostModel item) async {
    ...
    final list = SLPostModel.decode(raw);
    list.removeWhere((x) => x.id == item.id);
    await prefs.setString('info', SLPostModel.encode(list));

    if (!mounted) return;
    await _load();
  }

  void _review(SLPostModel item) {
    Navigator.push(
      context,
...
Future<void> _edit(SLPostModel item) async {
  ...
  void _goSearch() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (_) => SlopeidlistPage(id: "")),
    );
  }

  @override
  Widget build(BuildContext contezxt) {
    final items = _filtered;

    return Scaffold(
      appBar: AppBar(
        title: const Text('Inspected Slope'),
        centerTitle: true,
        actions: [
          IconButton(
            tooltip: 'Refresh',
